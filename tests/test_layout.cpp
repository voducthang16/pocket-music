...
void addLayoutTests(TestCases& tests) {
    ...
    tests.push_back(
        {"Home layout preserves the Hello Kitty artwork", [] {
             require(layout::contentRowsY <= 144, "first Home row must anchor the upper screen");
             require(layout::contentRowsX + layout::contentRowsWidth <= 620,
                     "Home rows must stay inside the quiet left region");
             require(layout::miniPlayerX == 0, "mini-player must align with the left screen edge");
             ...
             require(layout::contentRowsY + layout::contentRowHeight * 5 < layout::miniPlayerY,
                     "Home rows and mini-player must not overlap");
             ...
             require(layout::miniPlayerCoverY - layout::miniPlayerY ==
                         layout::miniPlayerY + layout::miniPlayerHeight -
                             (layout::miniPlayerCoverY + layout::miniPlayerCoverSize),
                     "mini-player cover must be vertically centered");
             require(
                 layout::footerTextY - (layout::miniPlayerY + layout::miniPlayerHeight) >= 18 &&
                     layout::footerTextY - (layout::miniPlayerY + layout::miniPlayerHeight) <= 24,
                 "footer must retain breathing room below the mini-player");
         }});
    tests.push_back({"Songs uses the Home safe content region", [] {
    ...
    tests.push_back(
        {"Now Playing preserves artwork and transport regions", [] {
             require(layout::nowPlayingCoverX + layout::nowPlayingCoverSize <= 620,
                     "Now Playing cover must stay clear of Hello Kitty");
             require(layout::nowPlayingCoverY + layout::nowPlayingCoverSize < layout::miniPlayerY,
                     "Now Playing cover must stay above the transport strip");
             require(layout::nowPlayingMetadataX + layout::nowPlayingMetadataWidth <= 620,
             ...
             require(layout::nowPlayingTitleWidth > layout::nowPlayingMetadataWidth &&
                         layout::nowPlayingMetadataX + layout::nowPlayingTitleWidth <= 820,
    ...
    tests.push_back(
        {"rounded surfaces keep solid side edges", [] {
             SDL_Surface* surface =
                 SDL_CreateRGBSurfaceWithFormat(0, 64, 48, 32, SDL_PIXELFORMAT_RGBA8888);
             require(surface != nullptr, "rounded rectangle surface must be created");
             SDL_Renderer* renderer = SDL_CreateSoftwareRenderer(surface);
             require(renderer != nullptr, "rounded rectangle software renderer must be created");
    ...
    tests.push_back(
        {"translucent rounded surfaces use one uniform fill", [] {
             SDL_Surface* surface =
                 SDL_CreateRGBSurfaceWithFormat(0, 80, 48, 32, SDL_PIXELFORMAT_RGBA8888);
             require(surface != nullptr, "rounded rectangle surface must be created");
             SDL_Renderer* renderer = SDL_CreateSoftwareRenderer(surface);
             require(renderer != nullptr, "rounded rectangle software renderer must be created");
